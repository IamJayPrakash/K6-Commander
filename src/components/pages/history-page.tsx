      ...
      reader.onload = (e) => {
        try {
          ...
          toast({
            ...
            description: error instanceof Error ? error.message : t('history.toast.importFailDescription'),
          });
        }
      };
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            <div className="flex gap-2">
             ...
             <div className="relative w-full md:w-auto md:max-w-xs">
                ...
                />
                {searchQuery && (
                    <Button variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => setSearchQuery('')} aria-label={t('history.clearSearchLabel')}>
                        <X className="h-4 w-4" />
                    </Button>
                )}
            </div>
            <AlertDialog>
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogAction onClick={handleDeleteAll} className="bg-destructive hover:bg-destructive/90">{t('history.clearAllDialog.confirm')}</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
          </div>
            ...
            {history.length === 0 ? (
                    ...
                    <TableBody>
                        {filteredHistory.map((item) => (
                            <TableRow key={item.id} data-testid={`history-item-${item.id}`}>
                                <TableCell className="font-medium truncate max-w-xs" title={item.config.url}>{item.config.url}</TableCell>
                                <TableCell>{format(new Date(item.timestamp), "PPp")}</TableCell>
                                <TableCell>
                                  <div className='flex gap-2 flex-wrap'>
                                    {item.config.runLoadTest && <Badge variant="outline"><Gauge className="mr-1 h-3 w-3"/>{t('form.k6SwitchLabel')}</Badge>}
                                    {item.config.runLighthouse && <Badge variant="outline"><ShieldCheck className="mr-1 h-3 w-3"/>{t('form.lighthouseSwitchLabel')}</Badge>}
                                    {item.config.runSeo && <Badge variant="outline"><BrainCircuit className="mr-1 h-3 w-3"/>{t('form.seoSwitchLabel')}</Badge>}
                                  </div>
                                </TableCell>
                                <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        <Button variant="ghost" size="icon" onClick={() => onLoad(item)} aria-label={t('history.actions.view', { url: item.config.url })} data-testid={`view-history-item-${item.id}`}><Eye className="h-4 w-4"/></Button>
                                        <Button variant="ghost" size="icon" onClick={() => onRerun(item.config)} aria-label={t('history.actions.rerun', { url: item.config.url })} data-testid={`rerun-history-item-${item.id}`}><Play className="h-4 w-4"/></Button>
                                        <AlertDialog>
                                            <AlertDialogTrigger asChild>
                                                <Button variant="ghost" size="icon" className="hover:text-destructive" aria-label={t('history.actions.delete', { url: item.config.url })} data-testid={`delete-history-item-trigger-${item.id}`}><Trash2 className="h-4 w-4"/></Button>
                                            </AlertDialogTrigger>
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                ...
                                                <AlertDialogAction onClick={() => handleDelete(item.id)} className="bg-destructive hover:bg-destructive/90">{t('history.deleteDialog.confirm')}</AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                                        </AlertDialog>
                                    </div>
             ...
             {filteredHistory.length === 0 && history.length > 0 && (
                <div className="flex flex-col items-center justify-center h-full p-4 text-center text-muted-foreground" data-testid="no-search-results-message">
                    ...
                    <Button variant="link" onClick={() => setSearchQuery('')}>{t('history.clearSearch')}</Button>
                </div>
            )}
          </ScrollArea>
        </CardContent>
