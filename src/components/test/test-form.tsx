...
import { Rocket, Trash2, Plus, Server, Settings, FileJson, ChevronsRightLeft } from 'lucide-react';
...
export default function TestForm({ initialConfig, onRunTest }: TestFormProps) {
  ...
  const onSubmit = async (data: TestFormValues) => {
    const config: TestConfiguration = {
      ...
      vus: data.testPreset === 'custom' ? data.vus! : TEST_PRESETS[data.testPreset].vus!,
      duration: data.testPreset === 'custom' ? data.duration! : TEST_PRESETS[data.testPreset].duration!,
      stages: data.testPreset === 'custom' ? data.stages! : TEST_PRESETS[data.testPreset].stages!,
      body: data.body || '',
    };
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Input placeholder="https://your-api.com/endpoint" {...field} className="pl-10 h-12 text-lg" />
                    </div>
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            <Button type="submit" size="lg" className="w-full" disabled={form.formState.isSubmitting}>
              <Rocket className="mr-2 h-5 w-5" /> Run Test
            </Button>
          </form>
        </Form>
      </CardContent>
