...
export async function POST(req: NextRequest) {
  try {
    ...
    const dockerArgs = [
      ...
      '-e',
      `K6_OUT=influxdb=http://influxdb:8086/k6,json=${outputPath}`,
      '-e',
      ...
      `BODY=${config.body || ''}`,
      '-e',
      `STAGES_JSON=${JSON.stringify(config.stages || [])}`,
      '-e',
      `VUS=${config.vus || ''}`,
      '-e',
      `DURATION=${config.duration || ''}`,
      '-e',
      `K6_INFLUXDB_TAGS_AS_FIELDS=testid`, // Custom tag for filtering in Grafana
      'grafana/k6',
      'run',
      '--tag',
      `testid=${testId}`,
