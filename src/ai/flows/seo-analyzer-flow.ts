...
const SeoAnalysisItemSchema = z.object({
  name: z
    .string()
    .describe(
      "The name of the SEO element being analyzed (e.g., 'Title Tag', 'Meta Description', 'Canonical URL')."
    ),
  value: z
    .string()
    .nullable()
    .describe('The current value of the SEO element as found on the page. Null if not found.'),
  status: z
    .enum(['pass', 'fail', 'warning'])
    .describe(
      "The status of the check: 'pass' if optimal, 'fail' if missing or critically flawed, 'warning' if present but could be improved."
    ),
  recommendation: z
    .string()
    .nullable()
    .describe(
      "A concise, actionable recommendation for improvement, generated by the AI. Null if status is 'pass'."
    ),
});

const SeoAnalyzerOutputSchema = z.object({
  analysis: z
    .array(SeoAnalysisItemSchema)
    .describe('An array of SEO analysis results for various on-page elements.'),
